...
async function create_session(access_token) {
  const url = `https://graph.facebook.com/v19.0/834177964858133/uploads?file_length=4446962&file_type=image/jpg&access_token=${access_token}`;
...
// async function uploadImage(filePath, uploadUrl, accessToken) {
...
async function initiate_upload(access_token, session_id) {
  ...
  console.log("Reponse of upload image: ", response);
...
async function create_message_templates(shop, segment_number) {
  const keys = await firebase_service.get_whatsapp_keys(shop);
  const session_id = await create_session(keys.whatsapp_access_token);
  console.log("Session id created: ", session_id);
  const file_handle = await initiate_upload(keys.whatsapp_access_token, session_id.id);
  console.log("File handle is: ", file_handle);
